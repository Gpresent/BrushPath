...
const LearnDeck: React.FC<LearnProps> = ({  }) => {
  ...
  useEffect(() => {
    if(userData) {
        ...
        const promises = Promise.all([getDeckFromID(id), getCharacterScoreData(userData?.email)]);
        promises.then(([deck ,score]) => {
            ...
            if(!deckData || !deckData?.characters) {
                setCharacters({ data: null, loading: false, error: "Deck data not found" });
                return;
            }
            if(scoreData === null || scoreData === undefined) {
                setCharacters({ data: null, loading: false, error: "Error getting character score" });
                return;
            }
            ...
            getCharsFromRefs(potentialCharacters, 0).then((fetchedChars) => {
                ...
                setCharacters({ data: fetchedChars
                    .map((character: any) => characterParser(character)).filter((character: Character | null): character is Character => character !== null), loading: false, error: "" });
