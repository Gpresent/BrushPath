...
export default function grade_svg(input: string, targetKanji: string) {
        ...
        .then(data => {
            ...
            for (let i = 0; i < inputCoords.length; i++) {
                ...
                const angleScore = (meanDiffs[i] / 60);
                ...
                const intersectionScore = Number(extraIntersections[i].length || missingIntersections[i].length);

                console.log("Stroke ", i + 1, " Angle Score: ", (1 - angleScore) * 100, "%, Length Score: ", ((1 - lengthScore) * 100).toPrecision(4), "%, Center Score: ", ((1 - centerScore) * 100).toPrecision(4), "%, Correct Intersections: ", intersectionScore === 1 ? "No" : "Yes");
